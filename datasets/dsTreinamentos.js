...
function createDataset(fields, constraints, sortFields) {
    try {
         ...
         */
        var arrayList = [], filtersTRN002 = [], filtersTRN002Table = [], filtersTRN004 = [];
        // Filters
        //var filtroAno = '', filtroOrigem = '', filtroArea = '', filtroDepartamento = '', filtroClassificacao = '';
        var filterPropor = '', filterProporTable = '', filterRequisitar = '';
        // Helpers
        var sqlLimit = false, limitReturn = 0;
        ...
        if ( filtersTRN002.length > 0 || filtersTRN002Table.length > 0 || filtersTRN004.length > 0) {
            ...
            for (var i = 0; i < solicitacoesDepartamentos.rowsCount; i++) {
                ...
                for (var x = 0; x < treinamentos.rowsCount; x++) {
                    ...
                    treinamentos.getValue(i, "treinamentoTbTreinamentos"),
                    treinamentos.getValue(i, "entidadeSugeridaTbTreinamentos"),
                    treinamentos.getValue(i, "cargaHorariaTbTreinamentos"),
                    treinamentos.getValue(i, "qtdeParticipanteTbTreinamentos"),
                    treinamentos.getValue(i, "matriculasNomesTbTreinamentos"),
                    treinamentos.getValue(i, "classificacaoTbTreinamentos"),
                    treinamentos.getValue(i, "estimativaTbTreinamentos"),
                    treinamentos.getValue(i, "valorGastoTbTreinamentos"),
            ...
            for (var i = 0; i < solicitacoesDepartamentos.rowsCount; i++) {
                var currentDocId = solicitacoesDepartamentos.getValue(i, 'documentid');
                var constraintDocId = DatasetFactory.createConstraint("documentid", currentDocId, currentDocId, ConstraintType.MUST);
                var treinamentos = DatasetFactory.getDataset('propor_treinamentos_anuais', null, [constraintTablePaiFilho, constraintDocId], null);
                for (var x = 0; x < treinamentos.rowsCount; x++) {
                    // propor code
                    customDataset.addRow(new Array('propor_treinamentos_anuais',
                    solicitacoesDepartamentos.getValue(i, "areaOrcamento"),
